...
function VerificaReg(codcliente,x,codproduto) {
    ...
    var constraints = new Array();
        ...
        if(data.values[0].VC_VERIFI == 0){
            console.log("O produto do cliente possui registro");
            FLUIGC.message.alert({
                message: 'O sistema verificou no Protheus e o produto do cliente já possui registro no cartório.\nPor favor acesse o Processo Análise Financeiro Cartório para procegrir para a proxima etapa',
                title: 'O produto do cliente já possui registro no Cartório',
                label: 'OK'
            },function(el, ev) {
            ...
            verificaAtivo(x)
            console.log("O cliente não possui registro no cartorio");            
       }
}
...
function historicoProcesso(idProcesso) {
    ...
    .catch(error => {
        log.info('Aconteceu algum erro!', error)
...
function verificaAtivo(x) {
    ...
    //Busca o dataset
    var data = DatasetFactory.getDataset("dsRegistroContratoCartorio", null, constraints, null);
    var count = data.values.length;
    if(count == 0)
        preencheDados(x);
    else{
        var verifica = 0,numSoli,link;
        for (let index = 0; index < count; index++) {
            ...
            for (let ct = 0; ct < count2; ct++) {
                if(data2.values[ct].active == 'true' || data2.values[ct].active == true){
                    link = 'https://thomasie156267.fluig.cloudtotvs.com.br/portal/p/1/pageworkflowview?app_ecm_workflowview_detailsProcessInstanceID='+numSoli;
                    verifica = 1;
                }else{
                    verifica = 0;
                }              
        ...
        if( verifica == 0){
            ...
            FLUIGC.message.error({
                title: 'AVISO: Lote já está sendo registrado',
                message: 'O sistema realizou uma busca no dado inserido e encontrou a <a href="'+link+'" target="_blank">Solicitação n°'+numSoli+'</a> ativa.',
                details: 'A verificação constatou que já existe uma solicitação ativa com esse lote e com isso não será possível a realização da inserção do mesmo, uma vez que não se pode realizar dois registro para o lote e cliente'
            }, function(el, ev) {
